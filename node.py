...
# to broadcast the transactions for the other nodes / open_transactions will now be visible for other nodes
...
def get_public_key():
    ...
    publicKey = required_fields["public_key"]
    privateKey = required_fields["private_key"] 
    if not all(field in values for field in required_fields):
        response = {
            'message': 'Required data is missing.'
        }
